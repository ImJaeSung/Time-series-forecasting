...
from datasets.market import build_market_information_from_constituents
#%%
...
class StockDataset(Dataset):
    def __init__(self, config, train=True):
        ...
        self.max_slow_period = max(
            [
                max(indicator_cfg.values())
                for indicator_cfg in config['indicators'].values()
                if len(indicator_cfg.values()) > 0
            ]
        )
        ...
        filtered_data = {
            ticker: baseline_data[ticker]
            for ticker in self.tickers
            if ticker in baseline_data
        }

        self.market_df, self.market_features = build_market_information_from_constituents(
            df=filtered_data,
            periods=[5, 10, 20, 30, 60],
            weighted_by_market_cap=False,   # market_cap 없으면 False
            prefix="sp500",
            fillna=False,
        )
        ...
        dataset_name = (
            dataset_dir
            + f"{config['market']}_{config['start_train']}_{config['lookback_length']}_with_market"
        )
        ...
        try:
            ...
            np.savez(
                ...
                X=self.X.numpy(),
                y=self.y.numpy(),
                buy_prob_threshold=np.array(buy_prob_threshold),
                sell_prob_threshold=np.array(sell_prob_threshold)
            )
    ...
    def split_train_test(self, config):
        ...
        for idx, sym in tqdm(enumerate(self.tickers), total=len(self.tickers), desc='Splitting dataset...'):
            ...
            _, df_x_train, df_y_train = self.preprocess(train_data, config)
    ...
    def get_data_baseline(self, symbol, start_date, end_date, lookback=0, lookforward=0):
        ...
        if lookback:
            concat_df = df.loc[:data.index[0]].tail(lookback)
            data = pd.concat([data, concat_df], axis=0).sort_index().drop_duplicates()

        if lookforward:
            concat_df = df.loc[data.index[-1]:].head(lookforward + 1)
            data = pd.concat([data, concat_df], axis=0).sort_index().drop_duplicates()
    ...
    def preprocess(self, source_df, config):
        ...
        if "obv" in config['indicators']:
            medium_period = config['indicators']["obv"]["medium_period"]
            slow_period = config['indicators']["obv"]["slow_period"]
        ...
        df = df.merge(
            self.market_df[["date"] + self.market_features],
            on="date",
            how="left"
        )

        features.extend(self.market_features)

        # -----------------------------
        # trim warmup / future leakage area
        # -----------------------------
        df = df.iloc[self.max_slow_period:-config['n_step_ahead']].copy()

        if not config['include_target'] and config['target_col'] in features:
    ...
    def gen_backtest_data(self, config, start_date, end_date):
        ...
        for idx, sym in tqdm(enumerate(self.tickers), total=len(self.tickers), desc='Generating backtest data...'):
