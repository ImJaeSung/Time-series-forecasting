...
from modules.utils import PParameter


class backbone_new_Linear(nn.Module):
    def __init__(
        self, c_in: int, context_window: int, target_window: int, head_dropout = 0, individual = False, revin = True, 
        ...
        self.cos = PParameter(cos, requires_grad=False)  # [L // 2 + 1, L]
        self.sin = PParameter(sin, requires_grad=False)
    ...
    def forward(self, z):
        ...
        norm = z.size()[-1]
        frequency = torch.fft.rfft(z, axis=-1)  # [B, D, L // 2 + 1]
        X_oneside = frequency / (norm) * 2

        basis_cos = torch.einsum('bkp,pt->bkpt', X_oneside.real, self.cos)  # [B, D, L // 2 + 1, L]
        basis_sin = torch.einsum('bkp,pt->bkpt', X_oneside.imag, self.sin)  # [B, D, L // 2 + 1, L]
...
class Flatten_Head_Linear(nn.Module):
    ...
    def forward(self, x):
        # x: [bs x nvars x d_model x patch_num] / [B, D, L // 2 + 1, L]
        if self.individual:
            x_out = []
            for i in range(self.n_vars):
            ...
            x = self.flatten(x)  # [B, D, L * (L // 2 + 1)]
            x = self.linear(x)  # [B, D, P]
        return x
...
class backbone_new_NLinear(nn.Module):
    def __init__(
        ...
        self.head = Flatten_Head_NLinear(
            self.individual, self.n_vars, self.head_nf, target_window, head_dropout=head_dropout
        )
        ...
        self.cos = PParameter(cos, requires_grad=False)
        self.sin = PParameter(sin, requires_grad=False)
    ...
    def forward(self, z):
        ...
        norm = z.size()[-1]
        frequency = torch.fft.rfft(z, axis=-1)
        X_oneside = frequency / (norm) * 2
    ...
    def __init__(self, individual, n_vars, nf, target_window, head_dropout=0):
        ...
        if self.individual:
            ...
            self.linear = nn.Sequential(
                nn.Linear(nf, 720 * 2), nn.Dropout(p=0.15), nn.ReLU(),
                nn.Linear(720 * 2, 720 * 2), nn.Dropout(p=0.15), nn.ReLU(),
...
class TSTiEncoder(nn.Module):
    ...
    def __init__(
        self, c_in, patch_num, patch_len, max_seq_len=1024, n_layers=3, d_model=128, n_heads=16, d_k=None, d_v=None,
        d_ff=256, norm='BatchNorm', attn_dropout=0., dropout=0., act="gelu", store_attn=False, key_padding_mask='auto', 
        padding_var=None, attn_mask=None, res_attention=True, pre_norm=False, pe='zeros', learn_pe=True, verbose=False, 
        **kwargs
    ):
        super().__init__()
...
class TSTEncoder(nn.Module):
    def __init__(
        self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=None,  norm='BatchNorm', attn_dropout=0., dropout=0., 
        ...
        self.layers = nn.ModuleList([
            TSTEncoderLayer(
                q_len, d_model, n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm, attn_dropout=attn_dropout, 
                dropout=dropout, activation=activation, res_attention=res_attention, pre_norm=pre_norm, store_attn=store_attn
...
class TSTEncoderLayer(nn.Module):
    def __init__(
        self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=256, store_attn=False, norm='BatchNorm', attn_dropout=0, dropout=0., 
        ...
        self.ff = nn.Sequential(
            nn.Linear(d_model, d_ff, bias=bias), get_activation_fn(activation), nn.Dropout(dropout),
...
class _MultiheadAttention(nn.Module):
    ...
    def forward(
        self, Q: Tensor, K: Optional[Tensor] = None, V: Optional[Tensor] = None, prev: Optional[Tensor] = None, key_padding_mask: Optional[Tensor] = None, 
...
class _ScaledDotProductAttention(nn.Module):
    ...
    def __init__(self, d_model, n_heads, attn_dropout=0., res_attention=False, lsa=False):
        ...
        self.scale = PParameter(torch.tensor(head_dim ** -0.5), requires_grad=lsa)
        self.lsa = lsa
...
class backbone_new_NPatchTST(nn.Module):
    def __init__(
        self, c_in: int, context_window: int, target_window: int, patch_num: int, stride: int, max_seq_len: Optional[int] = 1024, 
        ...
        store_attn: bool = False, pe: str = 'zeros', learn_pe: bool = True, fc_dropout:float=0., head_dropout = 0, padding_patch = None,
        ...
        self.backbone = TSTiEncoder(
            c_in, patch_num=patch_num, patch_len=patch_len, max_seq_len=max_seq_len, n_layers=n_layers, d_model=d_model,
            n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, attn_dropout=attn_dropout, dropout=dropout, act=act, 
