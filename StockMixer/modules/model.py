...
class StockMixer(nn.Module):
    def __init__(self, config):
        ...
        self.mixer = MultTime2dMixer(
            ...
            config['fea_num'], 
            scale_dim=config['scale_factor']
        )
        self.channel_fc = nn.Linear(config['fea_num'], 1)
        
        """Time mixing"""
        self.time_fc = nn.Linear(
            config['lookback_length'] * 2 + config['scale_factor'], 1
        )
        self.conv = nn.Conv1d(
            in_channels=config['fea_num'], 
            out_channels=config['fea_num'], 
            kernel_size=2, 
            stride=2
        )
        
        """Stock mixing"""
        self.stock_mixer = NoGraphMixer(config['stock_num'], config['market_num'])
