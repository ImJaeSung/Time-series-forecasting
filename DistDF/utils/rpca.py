    ...
    def _initialisation(
        ...
        if self.n_components is None:
            ...
            if n_components > min(X.shape):
                raise ValueError(
                    f"n_components must be <= min(X.shape)={min(X.shape)}, got {n_components}"
                )
        ...
        # Use scipy.linalg.svd if full rank, else svds
        if n_components == min(X.shape):
            U, Sigma, V = svd(X - S, full_matrices=False)
            U = U[:, :n_components]
            Sigma = np.diag(Sigma[:n_components])
            V = V[:n_components, :]
        else:
            U, s, V = svds(X - S, k=n_components)
            # svds returns unordered, sort singular values descending
            idx = np.argsort(s)[::-1]
            s = s[idx]
            U = U[:, idx]
            V = V[idx, :]
