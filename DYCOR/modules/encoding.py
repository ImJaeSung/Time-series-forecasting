...
class Mixer2d(nn.Module):
    def __init__(self, time_steps, channels):
        ...
        self.timeMixer = MixerBlock(time_steps, time_steps) # time mixing
        self.channelMixer = MixerBlock(channels, channels) # indicator mixing
...
class MultiScaleTimeMixer(nn.Module):
    def __init__(self, time_step, channel, scale_count=1):
        ...
        self.mix_layer = nn.ModuleList([nn.Sequential(
...
#%%
class StockEncoding(nn.Module):
    def __init__(self, config, time_steps, channels):
        super(StockEncoding, self).__init__()
        scale_dim = config['lookback_length'] // 2
        self.dlinear = DLinear(config)
        self.mixer = MultTime2dMixer(time_steps, channels, scale_dim=scale_dim)
        self.channel_fc = nn.Linear(channels, 1)
        self.time_fc = nn.Linear(time_steps + scale_dim, 1)
        self.conv = nn.Conv1d(
            in_channels=channels, out_channels=channels, kernel_size=2, stride=2)

        self.time_fc_final = nn.Linear(time_steps + scale_dim, 1)

    def forward(self, inputs):
        x = self.dlinear(inputs)
        x = x.permute(0, 2, 1)
        x = self.conv(x)
        x = x.permute(0, 2, 1)
        ...
        stock_embs = self.channel_fc(y).squeeze(-1)
