...
class StockDataset(Dataset):
    def __init__(self, config, train=True):
        ...
        self.max_slow_period = max(
            [max(indi.values()) for indi in config['indicators'].values() if indi.values()])
        #%%
        """Save the dataset"""
        dataset_dir = f"./assets/datasets/{config['market']}/"
        dataset_dir += "train/" if train else "test/"
        dataset_name = dataset_dir + f"{config['market']}_{config['lookback_length']}" 
        try:
    ...
    def get_data_baseline(self, symbol, start_date, end_date, lookback=0, lookforward=0):
        data_baseline = np.load("data/baseline_data_sp500.npy", allow_pickle=True).item()
    ...
    def sp500_tickers(self, start_date='2012-11-01', end_date='2022-01-01'):
        ...
        for t in data.keys():
            ticker = data[t]
            filtered = ticker[(ticker.index >= start_date)&(ticker.index <=end_date)]
