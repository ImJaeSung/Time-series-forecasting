...
import torch
import pandas as pd
from modules.utility import get_lead_lag_matrix
from modules.utility import get_ordered_clustering
import os

def get_lead_lag_cluster(train_data, corr_method):
    stock_indexes = []
    log_ret = {}
    ...
    lead_lag_matrix = get_lead_lag_matrix(
        log_ret, method="ccf_auc", max_lag=5, correlation_method=corr_method
    )
...
class HypergraphSnapshots:
    ...
    def _lead_lag_distance_incidence_matrix(self):
        lead_lag_path = f"data/lead_lag_edges_distance/sp500_leadlag_{self._start_train_date}.csv"
        if os.path.isfile(lead_lag_path):
            lead_lag = pd.read_csv(lead_lag_path, index_col=0).squeeze("columns")
        else:
            lead_lag = get_lead_lag_cluster(self._train_data_storage, corr_method='distance')
    ...
    def _lead_lag_kendall_incidence_matrix(self):
        lead_lag_path = f"data/lead_lag_edges_kendall/sp500_leadlag_kendall_{self._start_train_date}.csv"
        if os.path.isfile(lead_lag_path):
            lead_lag = pd.read_csv(lead_lag_path, index_col=0).squeeze("columns")
        else:
            lead_lag = get_lead_lag_cluster(self._train_data_storage, corr_method='kendall')
            lead_lag.to_csv(lead_lag_path)
        num_cluster = len(lead_lag.unique())
        incidence_matrix = np.zeros((len(self._symbols), num_cluster))
        for i in range(len(self._symbols)):
    ...
    def _lead_lag_mutual_information_incidence_matrix(self):
        lead_lag_path = f"data/lead_lag_edges_mutual_information/sp500_leadlag_mutual_information_{self._start_train_date}.csv"
        if os.path.isfile(lead_lag_path):
            lead_lag = pd.read_csv(lead_lag_path, index_col=0).squeeze("columns")
        else:
            lead_lag = get_lead_lag_cluster(self._train_data_storage, corr_method='mutual_information')
            lead_lag.to_csv(lead_lag_path)
        num_cluster = len(lead_lag.unique())
        incidence_matrix = np.zeros((len(self._symbols), num_cluster))
        for i in range(len(self._symbols)):
    ...
    def _lead_lag_pearson_incidence_matrix(self):
        lead_lag_path = f"data/lead_lag_edges_pearson/sp500_leadlag_pearson_{self._start_train_date}.csv"
        if os.path.isfile(lead_lag_path):
            lead_lag = pd.read_csv(lead_lag_path, index_col=0).squeeze("columns")
        else:
            lead_lag = get_lead_lag_cluster(self._train_data_storage, corr_method='pearson')
            lead_lag.to_csv(lead_lag_path)
        num_cluster = len(lead_lag.unique())
        incidence_matrix = np.zeros((len(self._symbols), num_cluster))
        for i in range(len(self._symbols)):
